...
serve(async (req) => {
  ...
  try {
    ...
    if (!whitelistEntry) {
      ...
      return new Response(
        JSON.stringify({ error: "Your email is not registered with your school. Please contact admin." }), 
    ...
    try {
      ...
      // Create student record with whitelisted data
      const { error: studentError } = await supabaseClient
        ...
        .insert({
          ...
          email,
          full_name: fullName,
          class_id: whitelistEntry.class_id,
          section: whitelistEntry.section,
          roll_no: whitelistEntry.roll_no,
          status: "active",
        });
    ...
    console.error("Error in student-signup function:", error);
    return new Response(
